...
export function dropHeader(rows: string[][]): string[][] {
  ...
  if (/\b(name|email|class|group|phone|nama|kelas|grup|kelompok|telepon)\b|姓名|班级|组|邮箱|电话/.test(first)) {
    return rows.slice(1)
  }
  return rows
}
...
const COLUMN_SYNONYMS: Record<string, string[]> = {
  ...
  email: ['email', 'e-mail', 'email address', 'surel', '邮箱', '电邮'],
  phone: ['phone', 'phone number', 'mobile', 'tel', 'telepon', 'no telepon', 'hp', '电话', '手机'],
...
export function mapCsvColumns(
  ...
  const hasHeader =
    /\b(name|email|class|group|phone|nama|kelas|grup|kelompok|telepon)\b|姓名|班级|组|邮箱|电话/.test(
      headerLine,
    )
