...
export function CoverageNeeded() {
  ...
  function onClaim(id: string) {
    claim.mutate(id, {
      onSuccess: () => toast.success('Kamu sekarang meng-cover grup ini'),
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Butuh coverage</CardTitle>
      </CardHeader>
            ...
            <div key={r.id} className="flex items-center justify-between gap-2 rounded-md border p-2">
              <div className="text-sm">
                <div>
                  ...
                  <span className="text-muted-foreground">
                    {' '}
                    · {r.class_name} · minggu {r.week_start_date}
                  </span>
                </div>
                {deadline !== null && (
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    !window.confirm(
                      `Konfirmasi kamu akan meng-cover ${r.group_name} · ${r.class_name}, pada ${sessionDayLabel(r.week_start_date, r.class_name)}?`,
                    )
                  )
                    return
                  onClaim(r.id)
              ...
              >
                {closed ? 'Ditutup' : 'Saya akan cover'}
              </Button>
            </div>
          )
        })}
